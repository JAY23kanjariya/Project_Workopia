...
import { FiArrowLeft, FiSave, FiX, FiLayers, FiAlertCircle, FiCheckCircle, FiEdit3 } from "react-icons/fi";
import categoryFormValidationSchema from "./categoryFormValidationSchema";

export default function CategoryForm({ isView = false }) {
    ...
    const validationSchema = categoryFormValidationSchema;
                ...
                try {
                    ...
                    if (res.data.success) {
                        ...
                        setInitialValues({
                            ...
                            status: c.status == true || c.status == 1 ? 1 : 0,
                        });
                    }
                } catch (err) {
    ...
    if (fetching) return <Loader text={isView ? "Retrieving category metadata..." : "Synchronizing category details..."} />;
        ...
        <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 py-10 px-4">
            <div className="flex items-center justify-between">
                ...
                <div className={`px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest ${isView ? "bg-indigo-50 text-indigo-600" : isEdit ? "bg-blue-50 text-blue-600" : "bg-emerald-50 text-emerald-600"
                    ...
                    {isView ? "Read Only" : isEdit ? "Modification Mode" : "New Entry"}
                </div>
            </div>

            <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden p-10">
                <div className="flex items-center gap-4 mb-8">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg ${isView ? "bg-indigo-600 shadow-indigo-100" : "bg-gradient-to-tr from-indigo-500 to-blue-600 shadow-indigo-200"
                        }`}>
                        <FiLayers className="w-7 h-7" />
                    </div>
                    <div>
                        <h2 className="text-2xl font-black text-gray-900 tracking-tight">
                            {isView ? "Category Details" : isEdit ? "Edit Category" : "Add New Category"}
                        </h2>
                        <p className="text-sm text-gray-400 font-medium italic">
                            {isView ? "System metadata for this industry classification." : "Capture the essence of this industry classification."}
                        </p>
                    </div>
                </div>
                ...
                <Formik
                    ...
                    enableReinitialize
                    validationSchema={isView ? null : validationSchema}
                                ...
                                <div className="relative group">
                                    <Field
                                        ...
                                        className={`w-full px-5 py-4 border rounded-2xl text-sm font-semibold transition-all outline-none focus:ring-4 ${isView ? "bg-slate-50 border-transparent text-gray-600 cursor-not-allowed" :
                                            errors.name && touched.name ? "bg-slate-50 border-red-200 focus:ring-red-50 focus:border-red-500" : "bg-slate-50 border-transparent focus:ring-indigo-50 focus:border-indigo-500 focus:bg-white"
                                            }`}
                                    />
                                    {!isView && errors.name && touched.name && (
                            ...
                            <div className="space-y-2">
                                ...
                                <Field
                                    ...
                                    className={`w-full px-5 py-4 border rounded-2xl text-sm font-semibold transition-all outline-none focus:ring-4 resize-none ${isView ? "bg-slate-50 border-transparent text-gray-600 cursor-not-allowed" :
                                        "bg-slate-50 border-transparent focus:bg-white focus:ring-indigo-50 focus:border-indigo-500"
                                        }`}
                                />
                            </div>
